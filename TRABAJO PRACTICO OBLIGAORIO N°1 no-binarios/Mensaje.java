...
public class Mensaje
{
    ...
    {
        if (linea == null)
        //la linea a agregar debe ser distinta de NULL
            throw new IllegalArgumentException("La linea a agregar no debe ser null.");
        if (linea.length() > LONG_MAX_LINEA)
        //la cantidad maxima de caracteres es de 80
            throw new IllegalArgumentException("Longitud invalida. La linea no debe tener más de 80 caracteres.");
        if (!esAscii(linea)) 
        //la linea solo tiene caracteres ASCII
            throw new IllegalArgumentException("La linea a agregar contiene caracteres no ascii.");
        if (pos<0 && pos>lineas.size())
        //la posicion donde se ponga la nueva linea debe estar dentro del ArrayList
            throw new IllegalArgumentException("la posicion debe estar entre 0 y la cantidad de lineas del mensaje.");
            
            //añade la linea en la posicion solicitada
    ...
    {
        if(otro==null)
        //la linea no debe ser NULL
        throw new IllegalArgumentException("no puede ser un mensaje vacio");
        //lo que hice aqui para compararlos fue llamar al metodo "toString" ya que si los mensajes imprimidos son 
        //iguales entonces todo es igual
        if(toString().equals(otro.toString()))
        {
           return true; 
        }
        else
