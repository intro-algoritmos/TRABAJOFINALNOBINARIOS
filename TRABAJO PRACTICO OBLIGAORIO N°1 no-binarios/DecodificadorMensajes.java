...
public class DecodificadorMensajes
{
    ...
    //profe en la parte de aqui lo que hice fue llamar al objeto codificadorMensaje ya que se me hizo ilogico decodificar
    //un mensaje que no esta codificado, entonces mande a codificar el mensaje para luego decodificarlo con el metodo.
    //por las dudas fue la forma en la que yo lo entendi, si esta mal tenemos otro que no tiene la llamada al objeto 
    
    public void decodificarMensaje() 
    {
        if(mensajeDecodificado != null) 
        //el mensaje no debe estar encriptado
        throw new IllegalArgumentException("el mensaje ya esta desencriptado");
        //aqui lo que hice fue llamar al objeto "CODIFICADORMENSAJES" para que me codifique el mensaje 
        //dado y asi, al momento de decodificarlo me de el mensaje no codificado
        CodificadorMensajes codificado = new CodificadorMensajes(mensajeADecodificar);
        codificado.codificarMensaje();
        mensajeADecodificar = codificado.obtenerMensajeCodificado();
        //aqui lo que hice fue que si la cantidad de lineas de mensaje a codificar es igual a 0 entonces 
        ...
        {
            //si la cantidad es mayor que 0 entonces se crea un for donde va a ir linea por linea 
            //decodificandolo con el metodo privado "DESENCRIPTARCADENA" y va a agregarle una linea al 
            //mensaje decodificado con la linea decodificada.
            mensajeDecodificado = new Mensaje();
            for(int i = 0 ; i<mensajeADecodificar.cantLineas(); i ++)
            {
                String linea = mensajeADecodificar.obtenerLinea(i);
                String lineaaux = desencriptarCadena(linea,codigoEncripcion);
                mensajeDecodificado.agregarLinea(lineaaux);
            }
        }
    }
    ...
    private String desencriptarCadena(String str, int[] codigo) {
        if(str == null) throw new IllegalArgumentException ("cadena vacia");
        if(codigo == null) throw new IllegalArgumentException ("codigo vacio");
        //lo que hice aqui fue crear una variable local para poder guardar ahi el resultado de la cadena 
        //el indiceCodigo funciona para poder ir en cada numero del arreglo, cuando se terminan los numeros, 
        //vuelve al primer numero del arreglo
        String resul = "";
        int indiceCodigo = 0;
        for (int i = 0; i < str.length(); i++) {
            // curr guarda la primera letra de la cadena
            char curr = str.charAt(i);
            // currEncreptado transforma el resultado de la letra contenida en curr menos el numero del codigo
            //en la posicion y lo divide por 128 quedandose con el resto
            char currEncriptado = (char) ((curr - codigo[indiceCodigo]) % 128);
            //guarda las letras que se van transformando
            resul = resul + currEncriptado;
            //el indice del codigo se suma con 1 para pasar a la siguiente posicion y luego lo divide
            //con la longitud del codigo para guardar el resto que es el numero de la posicion del arreglo
